(() => {
  ...
  networkStyle.textContent = `
    ...
    .history-badge{font-size:10px;letter-spacing:.8px;text-transform:uppercase;color:#7edcf0;border:1px solid rgba(91,178,225,.25);background:rgba(20,100,135,.18);padding:4px 8px;border-radius:99px;font-weight:800}
  `;
  ...
  const resizeNetwork = () => {
    ...
    particles = Array.from({length: count}, () => ({
      ...
      vx: (Math.random() - .5) * 1.95,
      vy: (Math.random() - .5) * 1.95,
      r: Math.random() * 2 + 1,
  ...
  // Persistent History tab for completed releases and their stored Feriha scenario.
  ...
  const writeHistory=s=>{try{localStorage.setItem(HISTORY_KEY,JSON.stringify(s))}catch{}};
  const escHistory=s=>String(s??'—').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const captureHistory=()=>{const cal=document.querySelector('#calendar');if(!cal)return;const store=readHistory();cal.querySelectorAll('.event').forEach(card=>{const title=card.querySelector('h4')?.textContent?.trim()||'';const when=card.querySelector('.event-title p')?.textContent?.trim()||'';if(!title)return;const vals=[...card.querySelectorAll('.metric')].map(m=>({label:m.querySelector('span')?.textContent?.trim()||'',value:m.querySelector('b')?.textContent?.trim()||''}));const actual=vals.find(v=>v.label==='Actual')?.value||'';const signal=card.querySelector('.signal b')?.textContent?.trim()||'WAIT';const confidence=card.querySelector('.signal small')?.textContent?.trim()||'';const reason=card.querySelector('.reason p')?.textContent?.trim()||'';const key=`${title}__${when}`;const old=store[key];const hasActual=actual&&actual!=='—'&&actual!=='-';const oldHasActual=old?.actual&&old.actual!=='—'&&old.actual!=='-';if(!old||hasActual||!oldHasActual)store[key]={title,when,values:vals,actual,signal,confidence,reason,savedAt:Date.now()}});writeHistory(store)};
  const renderHistory=()=>{captureHistory();const cal=document.querySelector('#calendar');if(!cal)return;const items=Object.values(readHistory()).sort((a,b)=>b.savedAt-a.savedAt);if(!items.length){cal.innerHTML='<div class="empty glass history-empty"><h3>No history yet</h3><p>Completed releases will appear here automatically after the calendar loads.</p></div>';return}cal.innerHTML=items.map(x=>{const vals=x.values||[];const prev=vals.find(v=>v.label==='Previous')?.value||'—';const forecast=vals.find(v=>v.label==='Forecast')?.value||'—';const actual=vals.find(v=>v.label==='Actual')?.value||x.actual||'—';const cls=/LONG/i.test(x.signal)?'long':/SHORT/i.test(x.signal)?'short':'wait';return `<article class="event glass"><div class="event-title"><div class="flag">🇺🇸</div><div><h4>${escHistory(x.title)}</h4><p>${escHistory(x.when)}</p></div></div><div class="metric"><span>Previous</span><b>${escHistory(prev)}</b></div><div class="metric"><span>Forecast</span><b>${escHistory(forecast)}</b></div><div class="metric"><span>Actual</span><b>${escHistory(actual)}</b></div><div><span class="impact">COMPLETED</span><div class="signal ${cls}"><b>${escHistory(x.signal||'WAIT')}</b><small>${escHistory(x.confidence||'')}</small></div></div><div class="expand"><div class="reason"><b>Feriha prediction / scenario</b><p>${escHistory(x.reason||'No stored scenario.')}</p></div></div></article>`}).join('')};
  const setupHistory=()=>{const filters=document.querySelector('.filters');const cal=document.querySelector('#calendar');if(!filters||!cal)return;if(!filters.querySelector('[data-filter="history"]')){const b=document.createElement('button');b.className='chip';b.dataset.filter='history';b.textContent='History';filters.appendChild(b);b.addEventListener('click',()=>{window.__ferihaHistory=true;document.querySelectorAll('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');renderHistory()})}if(!window.__ferihaHistoryObserver){const obs=new MutationObserver(()=>{if(!window.__ferihaHistory)captureHistory()});obs.observe(cal,{childList:true,subtree:true});window.__ferihaHistoryObserver=true;}};
  const boot=()=>{loadGoldPrice();setInterval(loadGoldPrice,INTERVAL);setupHistory();setInterval(setupHistory,1000)};
