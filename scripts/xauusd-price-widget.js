(() => {
  ...
  compactStyle.textContent = `
    ...
    .section-head{margin:11px 2px 5px!important}.section-head h3{font-size:17px!important}.section-head p{font-size:10px!important;margin:1px 0!important}.day{margin-bottom:8px!important}.datebar{margin:0 0 5px 5px!important;gap:5px!important}.datebar strong{font-size:15px!important}.datebar span{font-size:11px!important}
    .event{padding:10px 12px!important;border-radius:13px!important;gap:8px!important;margin-bottom:5px!important;min-height:76px!important;grid-template-columns:minmax(190px,1.6fr) repeat(3,minmax(72px,.55fr)) minmax(120px,.7fr)!important}
    .event:hover{transform:none!important;box-shadow:none!important}.event-title{gap:9px!important}.flag{width:32px!important;height:32px!important;border-radius:9px!important;font-size:14px!important}
    .event h4{font-size:17px!important;margin-bottom:1px!important;line-height:1.2!important}.event-title p,.metric span{font-size:12px!important;line-height:1.2!important}.metric b{font-size:14px!important;margin-top:1px!important}
    .impact{padding:3px 8px!important;font-size:10px!important}.signal{padding:8px 10px!important;border-radius:10px!important}.signal b{font-size:16px!important}.signal small{font-size:11px!important}
    .expand{display:none!important}.meter{height:4px!important}.meter-row{gap:5px!important}.score{font-size:12px!important}.footer{margin-top:9px!important;padding:9px!important;font-size:10px!important}
    #xauPriceWidget{min-width:175px!important;padding:7px 10px!important;border-radius:10px!important}
    #xauPriceWidget div:nth-child(2){font-size:20px!important}
  ...
  const render = (price, sourceUpdatedAt, live) => {
    ...
    node.innerHTML = `<div style="font-size:10px;letter-spacing:1px;text-transform:uppercase;color:#64808a;font-weight:800">XAU/USD <span style="color:${live ? '#07966f' : '#e34e65'}">● ${live ? 'LIVE' : 'OFFLINE'}</span></div><div style="font:800 20px Manrope;margin-top:1px">$${p.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</div><div style="font-size:10px;color:${changeColor}">${changeText}</div><div style="font-size:9px;color:#64808a;margin-top:1px">Last source tick: ${tick}</div>`;
  };
